...
check_deprecated_docker_compose_usage() {
    ...
    # Find script files that might contain deprecated docker-compose
    while IFS= read -r -d '' file; do
...
auto_fix_deprecated_usage() {
    ...
    echo "  2. Commit the changes: git add . && git commit -m 'fix: migrate from docker-compose to docker compose v2'"
